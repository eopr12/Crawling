...
def find_title_abstract(url_set, year_list):
    ...
    non_abs_url = []  # Q 이건 왜 크롤링? 
                      ...
                      # => 뉴욕타임즈에 기사가 없이 사진이 올라와있는 경우가 있어서 따로 url 수집하려고 만들어논 녀석
    ...
    print(year_list)  # Q year list를 여기서 print 하는 이유? => year가 잘 수집되고있나 중간에 체킹할려고 했던건데 지워도 무관!
...
# page_url은 처음 검색했을떄 뜨는 항목들의 url을 수집하여 url 리스트와 다음페이지가 존재한다면 다음페이지로 넘어가는 url을 return한다. (_url, next_link)

# 다음페이지가 존재한다면(next_link가 0) 이아니라면 다시한번 page_url함수에 다음 page가 들어가서 항목 url을 긁어온다.
...
# 엑셀 파일 다룰때 쓰는 파일 이었는데 클리앙에 이상한 데이터가 너무 많아서 쓰던 함수 여기선 무관!
...
#     이것도 뉴욕타임즈에서 쓰던것이라서 있는것임! 뉴욕타임즈에 기사의 요약본은 없고 기사 사진을 찍어놓은것들이 있는데 그것들의 url만 따로 수집하기위해 만들어 놓았음.
...
# 그냥 크롤링 돌리는데 얼마나 걸리는지 궁금해서 만든건데 
